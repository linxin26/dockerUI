		...
		initServerDetails : function(name) {
			$.ajax({
				...
				success : function(result) {
					...
					$("#spanCurrentFlow").text(result.flowData + "Mb");
					$("#spanflowDataSum").text(result.flowDataSum + "Mb");
//					console.log(result);
				},
				error : function(msg) {
					// console.log(msg);
		 ...
		 * @param scChart
		 */
		setTimer : function(name, scChart) {
			// dateType为function（初始化时）或日期为当天则开启定时器
			g.node.intervalId.push(setInterval(function() {
				$.ajax({
					type : "get",
					dataType : "json",
					url : "http://192.168.1.106:8080/GDCP-Monitor-Web/serverViewServlet?type=single&name=" + name,
					success : function(result) {
						// 10秒刷新数据
